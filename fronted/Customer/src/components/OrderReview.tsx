...
export default function OrderReview({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="w-full min-h-screen bg-[#F5F5F5] pt-14">
      <Header title="评价订单" onBack={onBack} />
      ...
      <div className="p-4 space-y-4">
         <div className="bg-white rounded-2xl p-6 shadow-sm flex flex-col items-center">
             ...
             <button onClick={handleSubmit} className="w-full bg-[#0085FF] hover:bg-blue-600 text-white font-bold py-3.5 rounded-full mt-6 active:scale-95 transition-all shadow-md">提交评价</button>
         </div>
      </div>
    </div>
  )
