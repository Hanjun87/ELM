...
export default function BottomNav({ activeTab, onChange, cartCount }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md z-50 bg-white shadow-[0_-8px_24px_rgba(0,0,0,0.04)] pb-safe rounded-t-[20px] border-t border-gray-100">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center w-full h-full relative transition-all duration-200 ${isActive ? 'text-[#0085FF] bg-blue-50/50 rounded-2xl scale-105' : 'text-gray-400 hover:text-gray-600'}`}
            >
              <div className="relative">
                <Icon size={24} className={isActive ? 'fill-blue-100' : ''} strokeWidth={isActive ? 2.5 : 2} />
