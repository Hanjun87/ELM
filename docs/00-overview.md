...
|------|------|--------|--------|---------|
| Customer 小程序 | `apps/customer/` | Taro 4 + React + TS | 首页 / 订单 / 购物车 / 我的 | ✅ 完整对接 |
| Merchant 小程序 | `apps/merchant/` | Taro 4 + React + TS | 订单 / 商品 / 数据 / 我的 | ✅ 订单收发、商品 CRUD、店铺设置对接；数据看板为 Mock |
| Rider 小程序 | `apps/rider/` | Taro 4 + React + TS | 待接单 / 我的配送 / 历史 / 我的 | ✅ 抢单、取餐、送达、开工状态对接 |
...
构建：`cd apps/<app> && npm install && npm run build:weapp`，产物在 `dist/`，用微信开发者工具导入对应 `apps/<app>` 目录（AppID 可用测试号）。连本地后端需在「详情 → 本地设置」勾选「不校验合法域名」。
...
| 已废弃应用 | 目录 | 取代者 |
|-----------|------|--------|
| ~~Customer (Web)~~ | `[deleted]/` | `apps/customer/` |
| ~~Merchant (Web)~~ | `[deleted]/` | `apps/merchant/` |
| ~~Rider (Web)~~ | `[deleted]/` | `apps/rider/` |
